...
async function render(
  ...
  const context = await chromium.launchPersistentContext(userDataDir, {
    ...
    // The profile directory exists before the browser does; if the launch
    // itself fails there is no context to close, only a directory to remove.
    fs.rmSync(userDataDir, { recursive: true, force: true });
    throw err;
  });

  // Everything from here to the end of the render is wrapped so the browser
  // and its profile are released on every exit path. They used to be torn
  // down twice — once on the happy path, once when a segment failed — which
  // left any other failure (setup, title card, frame assembly, the merge)
  // holding a Chromium process and a directory in /tmp. Six of those were
  // still on disk after a run of failed renders.
  ...
  const releaseBrowser = async (): Promise<void> => {
    ...
    if (cdpSession) {
      ...
      // Frames already in flight still arrive after the stop; writes are
      // synchronous, so this only waits on delivery of events the browser
      // has already sent.
  ...
  cdpSession = cdp;
    ...
    for (const segment of playbook.segments) {
      ...
      const result = await runSegment(page, segment, {
        ...
        onActionError: async () => {
          // Best-effort. When the failure was the page or browser going away,
          // screenshotting throws too — and because this runs inside the
          // executor's catch, that second throw replaces the first one. The
          // render then reports "Target page has been closed" and the actual
          // reason the action failed is gone.
          try {
            await page.screenshot({
              path: path.join(outputDir, `error-${segment.id}.png`),
            });
          } catch (err) {
            console.log(`  (no se pudo capturar la pantalla: ${String(err).split("\n")[0]})`);
          }
        },
      });
    ...
    // The picture is complete; let go of the browser before the ffmpeg work
    // so a long encode is not holding a Chromium process open.
    await releaseBrowser();
    ...
    await releaseBrowser();
    // Frames are an intermediate artifact; on the happy path they are already
    // gone by here, and on a failure they are hundreds of megabytes sitting in
    // the user's demo directory. The error screenshot is the thing worth
    // keeping.
    fs.rmSync(framesDir, { recursive: true, force: true });
  }
}
